...
// このような例は既に[ワーカープール](worker-pools)で紹介した。
//
// 別のやり方もある。
// ここでは、`sync/atomic` パッケージにある、複数のゴルーチンからアクセスされる _アトミックなカウンタ_ を使う方法を紹介する。
...
func main() {
	...
	// 更新を並行に実行するため、50 個のゴルーチンを起動する。
	// 各ゴルーチンは約 1 ミリ秒ごとにカウンタの値を 1 増やす。
	for i := 0; i < 50; i++ {
		go func() {
			for {
